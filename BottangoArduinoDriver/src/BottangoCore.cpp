...
// enter exported animation if required
    ...
    void executeCommand(char *commandString)
    {
        ...
        if (Callbacks::handleExternalCommand(splitCommandBuffer, commandCount))
        {
            return;
        }
    ...
    bool externalCommandIsAllowed(char *commandString)
    {
        ...
        if (offline)
        {
            ...
            strncpy(commandCopy, commandString, MAX_COMMAND_LENGTH - 1);
            commandCopy[MAX_COMMAND_LENGTH - 1] = '\0';

            byte commandCount = 0;
            bool splitSuccess = splitIntoBuffer(commandCopy, commandCount);
            if (!splitSuccess)
            {
            ...
            if (Callbacks::isExternalCommandAllowed(commandName))
            {
                return true;
            }
